...
Abstract mainnet (chain `2741`) is the default and the live network. The verified, canonical address list is published at `/contracts/addresses`. If anything below disagrees with that page, that page wins. The Abstract testnet profile stays available for development.
