...
- **Jail.** Going to jail subtracts `(reputation × jailRepPenaltyPercent) / 100`, capped at `jailRepPenaltyCap`. The current defaults are **10% of current reputation, capped at 50 rep absolute**. Jail also confiscates a percentage of one drug from your stash (`jailDrugConfiscationPercent`). Both numbers are read from the core config and may move with rebalances.
...
- **PVP.** Until you cross `pvpMinReputation`, the PVP tab is read-only — you can browse and scout but cannot commit attacks.
