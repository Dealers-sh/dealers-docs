...
Testnet contract addresses have been published. The full list is available on [/contracts/addresses](/contracts/addresses), and every address is verified on the Abstract testnet explorer. The audit is currently in progress, with status tracked on [/contracts/audit](/contracts/audit). Mainnet launch is gated on the audit closing cleanly.
